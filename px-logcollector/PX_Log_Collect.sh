...
if [ $# -lt 1 ]; then
  echo "Worker IP is  not specified . The log will be collected form all avialble workers"
  ALLWORKERS="true"
elif [ $1 = --worker ]; then
   WORKER_IP=$2
   NoOfWorkers=$#
...
kubectl -n $NAMESPACE get Pod -l app=runevery -l runevery=$DS_NAME -o jsonpath='{range .items[*]}{.metadata.name} {.spec.nodeName}{"\n"}{end}' | sort -k2 | while read podname nodename; do
        PXLOGS="/tmp//pxlogs/${nodename}"
        mkdir -p $PXLOGS
         kubectl  cp  ${NAMESPACE}/${podname}:/tmp/pod/PXstatus $PXLOGS
         kubectl  cp  ${NAMESPACE}/${podname}:/tmp/pod/PXclusterstatus $PXLOGS
         kubectl  cp  ${NAMESPACE}/${podname}:/tmp/pod/PXjournallog $PXLOGS
         kubectl  cp  ${NAMESPACE}/${podname}:/tmp/pod/diags.tar.gz $PXLOGS
done
}
...
PXLOGS="/tmp//pxlogs/${WORKER_IP}"
        mkdir -p $PXLOGS
        echo "pod name = ${POD}"
         kubectl  cp  ${NAMESPACE}/${POD}:/tmp/pod/PXstatus $PXLOGS
         kubectl  cp  ${NAMESPACE}/${POD}:/tmp/pod/PXclusterstatus $PXLOGS
         kubectl  cp  ${NAMESPACE}/${POD}:/tmp/pod/PXjournallog $PXLOGS
         kubectl  cp  ${NAMESPACE}/${POD}:/tmp/pod/diags.tar.gz $PXLOGS
}
